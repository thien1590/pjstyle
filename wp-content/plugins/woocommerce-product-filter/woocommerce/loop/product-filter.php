				...
				<div class="prdctfltr_filter prdctfltr_byprice<?php echo $customization['class']; ?>"  data-filter="pf_byprice">
					...
					if ( !empty( $filter_customization ) && isset( $filter_customization['settings'] ) && is_array( $filter_customization['settings'] ) ) {
						...
						foreach( $filter_customization['settings'] as $k => $v ) {
							...
							$add_k = WC_Prdctfltr::price_to_float($pf_custom_ranges[0])-WC_Prdctfltr::price_to_float($pf_custom_ranges[1]);
							$catalog_ready_price[$add_k] = ( $add_num !== '' ? $add_num : '' );
						}
						...
						for ( $i = 0; $i < $curr_price_limit; $i++ ) {
							...
							$curr_prices[$i] = WC_Prdctfltr::price_to_float( strip_tags( wc_price( apply_filters( 'wcml_raw_price_amount', $min_price ) ) ) ) . '-' . ( ($i+1) == $curr_price_limit ? '' : WC_Prdctfltr::price_to_float( strip_tags( wc_price( apply_filters( 'wcml_raw_price_amount', $max_price ) ) ) ) );
		...
		<?php
			...
			default :
				...
				if ( !empty( $pf_terms ) && !is_wp_error( $pf_terms ) ) {
					...
					$curr_include = $curr_fo['settings']['include'];
					if ( empty( $curr_include ) ) {
						foreach ( $pf_terms as $term ) {
							$curr_include[] = $term->slug;
						}
					}
