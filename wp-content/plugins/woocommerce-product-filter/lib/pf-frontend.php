<?php
	...
	class WC_Prdctfltr {
		...
		function __construct() {
			...
			add_filter( 'woocommerce_product_query', array( &$this, 'prdctfltr_wc_query' ), 999999, 2 );
			add_filter( 'woocommerce_product_query', array( &$this, 'prdctfltr_wc_tax' ), 999999, 2 );
			...
			if ( ( isset( $pf_activated['min_price'] ) || isset( $pf_activated['max_price'] ) ) !== false || ( isset( $pf_activated['rng_min_price'] ) && isset( $pf_activated['rng_max_price'] ) ) !== false || ( isset( $pf_activated['sale_products'] ) || isset( $query->query['sale_products'] ) ) !== false ) {
				if ( self::$wc_version === false ) {
					...
					if ( ( isset( $pf_activated['sale_products'] ) || isset( $query->query['sale_products'] ) ) !== false ) {
						$curr_args['post__in'] = wc_get_product_ids_on_sale();
					}
		...
		public static function prdctfltr_wc_tax( $query, $that ) {
			...
			$pf_activated = $prdctfltr_global['active_taxonomies'];;
			$pf_tax_query = apply_filters( 'prdctfltr_tax_query', self::$settings['tax_query'] );
			...
			if ( !empty( $pf_tax_query ) ) {
				...
				$query->query_vars['tax_query'] = $pf_tax_query;
		...
		public static function get_prices() {
			...
			if ( isset( $_max_price ) ) {
				$_max_price = floatval( $_max_price ) + apply_filters( 'prdctfltr_max_price_margin', 0.999 );
			}
					...
					switch( $k ) {
						...
						case 'min_price' :
							if ( isset( $pf_activated['min_price'] ) && $pf_activated['min_price'] !== '' ) {
								...
								echo strip_tags( wc_price( $min_price ) ) . ( isset( $max_price ) && $max_price > 0 ? ' - ' . strip_tags( wc_price( $max_price ) ) : $max_price );
						...
						case 'rng_min_price' :
							if ( isset( $pf_activated['rng_min_price'] ) && $pf_activated['rng_min_price'] !== '' ) {
								$prefix  = self::get_range_value( 'price', 'prefix' );
								$postfix = self::get_range_value( 'price', 'postfix' );
								...
								echo __( 'Price range', 'prdctfltr' ). ' ' . $prefix  . strip_tags( wc_price( apply_filters( 'wcml_raw_price_amount', $min_price ) ) ) . $postfix . ' &rarr; ' . $prefix . strip_tags( wc_price( apply_filters( 'wcml_raw_price_amount', $max_price ) ) ) . $postfix;
						...
						default :
							if ( substr( $k, 0, 4 ) == 'rng_' ) {
								...
								$prefix  = self::get_range_value( $true_val, 'prefix' );
								$postfix = self::get_range_value( $true_val, 'postfix' );

								echo __( 'From', 'prdctfltr' ) . ' ' . $prefix . $curr_selected['min'] . $postfix . ' ' . __( 'to' , 'prdctfltr' ) . ' ' . $prefix . $curr_selected['max'] . $postfix;
								...
								foreach( $curr_selected as $selected ) {
									if ( !isset( $active_filters[$k] ) || !empty( $active_filters[$k] ) && !in_array( $selected, $active_filters[$k] ) ) {
										continue;
									}
