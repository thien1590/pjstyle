<?php
	...
	class WC_Prdctfltr_Shortcodes {
		...
		public static function prdctfltr_respond_550() {
			...
			if ( $opt['pf_step'] == 0 ) {
				...
				foreach ( $curr_filters as $cfk => $cfv ) {
					...
					if ( $opt['pf_set'] == 'shortcode' && is_array( $opt['pf_shortcode'] ) && array_key_exists( $cfk, $opt['pf_shortcode'] ) && $opt['pf_shortcode'][$cfk] == $cfv ) {
						continue;
					}
		...
		public static function make_query() {
			...
			remove_filter( 'parse_tax_query', 'WC_Prdctfltr::sc_wc_tax', 999999, 1 );

			$products->is_search = false;

			if ( isset( $pf_activated['products_per_page'] ) ) {
				$pf_per_page = floatval( $pf_activated['products_per_page'] );
			}
